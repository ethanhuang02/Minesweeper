...
//輸出沒踩到地雷的盤面  
void printfield(int field[][9],int mark[][9],int count[][9],int x,int y,int won){
	system("CLS");//刷新畫面 
	cout<<"\n☆歡迎來到踩地雷☆\n\n";
	for(int i=0;i<9;i++){
		if(i>0)	cout<<i<<" ";
	...
	do{
		...
		if(input!=open&&input!=note||x<1||x>8||y<1||y>8){
			...
			if(field[x][y]==87){//輸出踩到地雷的盤面 
				...
				printfield(field,mark,count,x,y,won);			
				//判斷勝負							
				if(won==1){
					cout<<"\n你贏了!!灑花~~~~~(*￣▽￣)/‧☆*'*-.,_,.-*'*-.,_☆\n\n"<<endl;
					break;
